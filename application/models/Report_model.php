...
class Report_model extends CI_Model {
    ...
    public function get_stockout_summary($province_id, $district_id, $year, $month = 12) {
        ...
        // ================================
        // Ambil total Puskesmas yang stockout (UNIK)
        // ================================
    ...
    //         -- Kolom untuk cek apakah puskesmas sudah melakukan imunisasi
    //         IF(EXISTS(
    //             SELECT 1 
    //             FROM immunization_data id 
    //             WHERE id.puskesmas_id = pd.id 
    //             AND id.year = {$year} 
    //             " . ($month !== 'all' ? "AND id.month <= {$month}" : "") . " 
    //             " . ($province_id === 'targeted' && !empty($province_ids) ? "AND id.province_id IN (" . implode(",", $province_ids) . ")" : "") . "
    //             " . ($province_id !== 'all' && $province_id !== 'targeted' ? "AND id.province_id = {$province_id}" : "") . "
    //             " . ($city_id !== 'all' ? "AND id.city_id = {$city_id}" : "") . "
    //         ), 1, 0) AS has_immunization,
            
    //         -- Kolom untuk cek apakah puskesmas sudah di supervisi (SS)
    //         IF(EXISTS(
    //             SELECT 1 
    //             FROM supportive_supervision ss 
    //             WHERE ss.puskesmas_id = pd.id 
    //             AND ss.year = {$year} 
    //             " . ($month !== 'all' ? "AND ss.month <= {$month}" : "") . " 
    //             " . ($province_id === 'targeted' && !empty($province_ids) ? "AND ss.province_id IN (" . implode(",", $province_ids) . ")" : "") . "
    //             " . ($province_id !== 'all' && $province_id !== 'targeted' ? "AND ss.province_id = {$province_id}" : "") . "
    //             " . ($city_id !== 'all' ? "AND ss.city_id = {$city_id}" : "") . "
    //         ), 1, 0) AS has_ss,
            
    //         -- Subquery untuk mendapatkan kategori good pada puskesmas di supervisi
    //         (SELECT SUM(ss.good_category_puskesmas) 
    //         FROM supportive_supervision ss 
    //         WHERE ss.puskesmas_id = pd.id 
    //         AND ss.year = {$year} 
    ...
    //         ) AS category_good
    //     ");
    //     $this->db->from('puskesmas pd');
        
    //     // Filter berdasarkan provinsi yang ditargetkan
    //     if ($province_id === 'targeted') {
    //         if (!empty($province_ids)) {
    //             $this->db->where_in('pd.province_id', $province_ids);
    //         } else {
    //             return []; // Jika tidak ada province yang ditargetkan, kembalikan array kosong
    //         }
    //     } elseif ($province_id !== 'all') {
    //         $this->db->where('pd.province_id', $province_id);
    //     }
        
    //     // Filter berdasarkan kota jika diberikan
    //     if ($city_id !== 'all') {
    //         $this->db->where('pd.city_id', $city_id);
    //     }

    //     // Ambil hasil query
    //     $query = $this->db->get()->result_array();
        
    //     // Proses untuk menghitung atau memformat data lebih lanjut (jika perlu)
    //     foreach ($query as &$row) {
    //         // Konversi untuk memastikan bahwa nilai-nilai tersebut adalah integer
    //         $row['has_immunization'] = (int) $row['has_immunization'];
    //         $row['has_ss'] = (int) $row['has_ss'];
    //         $row['category_good'] = $row['category_good'] ? $row['category_good'] : null; // Jika null, set ke null
    //     }
    ...
    public function get_puskesmas_dpt_stock_out_table($province_id = 'all', $city_id = 'all', $year = 2025, $month = 12) {
        ...
        // ================================
        $this->db->select('id AS province_id, name_id AS province_name');
        $this->db->from('provinces');
        $provinces = $this->db->get()->result_array();
    
        // ================================
        // Query untuk menghitung total Puskesmas yang mengalami stockout per provinsi
        // ================================
        $this->db->select('
            ss.province_id,
            COUNT(DISTINCT ss.puskesmas_id) AS total_stockout
        ');
        ...
        // Group by province_id untuk menghitung stockout per provinsi
        $this->db->group_by('ss.province_id');
        $stockout_data = $this->db->get()->result_array();
        
        // ================================
        // Query untuk menghitung total Puskesmas per provinsi (aktif)
        // ================================
        $this->db->select('
            p.province_id,
        ...
        $this->db->join('provinces pr', 'p.province_id = pr.id', 'left');
        ...
        // Group by province_id untuk menghitung total Puskesmas per provinsi
        $this->db->group_by('p.province_id');
        $total_puskesmas_data = $this->db->get()->result_array();
        
        // ================================
        // Gabungkan hasil data stockout dan total Puskesmas per provinsi
        // ================================
        foreach ($provinces as $province) {
            $province_id = $province['province_id'];
            $province_name = $province['province_name'];
            
            // Inisialisasi nilai stockout dan total_puskesmas untuk setiap provinsi
            $total_stockout = 0;
            $total_puskesmas = 0;
            
            // Cek apakah ada data stockout untuk provinsi ini
            ...
            // Cek total Puskesmas untuk provinsi ini
            foreach ($total_puskesmas_data as $puskesmas) {
                if ($puskesmas['province_id'] == $province_id) {
                    $total_puskesmas = $puskesmas['total_puskesmas'];
                    break;
                }
            }
    ...
    public function get_puskesmas_dpt_stock_out_table_by_province($province_id = 'all', $city_id = 'all', $year = 2025, $month = 12) {
        ...
        // ================================
        $this->db->select('c.id AS city_id, c.name_id AS city_name, p.id AS province_id, p.name_id AS province_name');
        $this->db->from('cities c');
        $this->db->join('provinces p', 'c.province_id = p.id', 'left');
        ...
        // ================================
        $this->db->select('p.city_id, COUNT(p.id) AS total_puskesmas');
        $this->db->from('puskesmas p');
