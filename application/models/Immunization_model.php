...
class Immunization_model extends CI_Model {
    ...
    public function get_dpt1_by_province($year = 2025, $max_month = 1, $province_filter = 'all') {
        ...
        $target_subquery = $this->db->select('province_id, SUM(dpt_hb_hib_1_target) AS target_district')
    ...
    public function get_zd_comparison_data($year = 2025, $max_month = 1, $province_id = 'all', $district_id = 'all')
    {
        ...
        if ($province_id === 'all' || $province_id === 'targeted') {
            ...
            // Ambil data ZD 2024 per provinsi
            $this->db->select('
                provinces.id AS id,
                provinces.name_id AS district,
                SUM(zd_cases_per_month.dpt_hb_hib_1) AS coverage_2024,
                SUM(zd_cases_per_month.target) AS full_target_2024
            ');
            ...
            // Ambil data imunisasi tahun $year
            $this->db->select('
                provinces.id AS id,
                provinces.name_id AS district,
                SUM(target_immunization.dpt_hb_hib_1_target) AS full_target_selected,
            ...
            foreach ($zd2025 as $row) {
                ...
                $full_target_2024 = $zd2024_map[$id]['full_target_2024'] ?? 0;
                $coverage_2024 = $zd2024_map[$id]['coverage_2024'] ?? 0;
                $target_2024 = $full_target_2024 * $max_month / 12;
                $zd_2024 = $target_2024 - $coverage_2024;

                $full_target_selected = $row['full_target_selected'] ?? 0;
                $coverage_selected = $row['coverage_selected'] ?? 0;
        ...
        // =====================
            ...
            return []; // Sesuai permintaan, dikosongkan
        }
    }
